...
if __name__ == '__main__':
  ...
  #mesh_dataset = hc.datasets.ToscaMesh(obj_class='horse')
  
  #SCAN = '%s/MPI-FAUST/training/scans/tr_scan_{0:03d}.ply' % (PATH_TO_DATA)
  REG = '%s/TOSCA/mesh/horse{}.ply' % (PATH_TO_DATA)
  #FAUST_DESC = '%s/faust/faust_descriptors/{0:03d}.mat' % (PATH_TO_DATA)
  rotation_path = '%s/TOSCA/render_rotations' % (PATH_TO_DATA)
  render_path = '%s/TOSCA/scans' % (PATH_TO_DATA)
  OBJ = '%s/{0:03d}_{1:03d}.obj' % (render_path)
  MESH_CORRES = '%s/TOSCA/mesh/horse{}.corres' % (PATH_TO_DATA)
  ...
  rest_mesh = vis.getTriangleMesh(np.array(rest_mesh.vertices), np.array(rest_mesh.triangles).astype(np.int32))
  ...
  for scan_id in range(args.offset, args.offset+args.length):
    ...
    gt_corres = np.arange(Nreg) #gt_corres[:, 0]
