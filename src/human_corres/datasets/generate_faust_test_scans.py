...
if __name__ == '__main__':
  ...
  parser.add_argument('--num_views', type=int, default=200,
                      help='how many rendering views [default: 200]')
  parser.add_argument('--offset', type=int, default=0,
                      help='starting index [default: 0]')
  parser.add_argument('--length', type=int, default=0,
                      help='number of meshes to render [default: 1]')
  args = parser.parse_args()
  SCAN = '%s/MPI-FAUST/test/scans/test_scan_{0:03d}.ply' % (PATH_TO_DATA)
  #REG = '%s/MPI-FAUST/test/registrations/tr_reg_{0:03d}.ply' % (PATH_TO_DATA)
  #FAUST_DESC = '%s/faust/faust_descriptors/{0:03d}.mat' % (PATH_TO_DATA)
  rotation_path = '%s/faust-test/render_rotations' % (PATH_TO_DATA)
  render_path = '%s/faust-test/scans' % (PATH_TO_DATA)
  OBJ = '%s/{0:03d}_{1:03d}.obj' % (render_path)
  #MESH_CORRES = '%s/faust-test/training/registrations/test_reg_{0:03d}.corres' % (PATH_TO_DATA)
