...
def train(model, loader, optimizer, lr_scheduler, args, epoch):
  ...
  with progressbar.ProgressBar(max_value=len(loader), widgets=widgets) as bar:
    for i, data in enumerate(loader):
      ...
      optimizer.step()
      lr_scheduler.step()
      #corres = correspondence(out.detach(), template_feats)
      with torch.no_grad():
        pred = out.max(1)[1]
        l2dist = (((template_points[pred, :] - template_points[data.y, :]) ** 2).sum(-1)
                 .sqrt().mean())
  ...
  with torch.no_grad():
    with progressbar.ProgressBar(max_value=len(loader), widgets=widgets) as bar:
      for i, data in enumerate(loader):
        ...
        loss = F.nll_loss(out, data.y)
        pred = out.max(1)[1]
        l2dist = (((template_points[pred, :] - gt_points) ** 2).sum(-1)
                 .sqrt().mean())
        total_l2dist += l2dist.item()
        total_loss += loss.item()
        bar.update(
